...
from Model.Datalayer import (
    ...
    # VIP
    get_vip_by_khach_hang, create_or_update_vip,get_giamgia_vip,
    # SanPham
    get_san_pham_by_id,
)
...
def dat_hang(khach_hang_id, dia_chi_giao, phuong_thuc_thanh_toan,
    ...
    # 3. Mã giảm giá VIP
    giam_gia_vip = get_giamgia_vip(khach_hang_id)
    if giam_gia_vip > 0:
        giam_gia += giam_gia_vip*tong_tien/Decimal('100')
...
def get_bao_hanh_by_don_hang_and_khach(khach_hang_id):
    from Model.Datalayer import get_bao_hanh_by_khach_hang
